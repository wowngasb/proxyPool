define(["jquery", "underscore", "views/generic/datatablepage", "models"],function($, _, DataTablePage, Models) {
  ...
  return DataTablePage.extend({
    ...
    hookDrawCallback: function (oSettings) {
      self.jobData = {};
    ...
    initFilters: function() {
      this.filters = {
        ...
        "time_s": this.options.params.time_s||"",
        "time_e": this.options.params.time_e||"",
      };
    },
        ...
        "aoColumns": [

          {
            ...
            "mData": function ( source /*, val */) {
              var queued_t = parseInt(source._id.substring(0, 8), 16);
              var started_t = moment.utc(source.datestarted).valueOf() / 1000;
              var started_str = started_t ? "<br/><br/>started: <small>"+dms.date('m-d H:i:s', started_t)+"</small>" : '';
              return "<a href='/#jobs?path="+source.path+"'>"+source.path+"</a>"+
                "<br/><br/><a href='/#jobs?id="+source._id+"'><small>"+source._id+"</small></a>" + 
                "<br/><br/>queued: <small>"+dms.date('m-d H:i:s', queued_t)+"</small>" + started_str;
            }
          },
          {
            "sTitle": "Params",
          ...
          },
          {
            ...
            "mData":function(source, type/*, val*/) {
              ...
              if (type == "display") {
                var queued_t = parseInt(source._id.substring(0, 8), 16);
                var display = [
                  "queued  <span title='" + moment.utc(1000 * queued_t).fromNow() + dms.date(' Y-m-d H:i:s', queued_t) + "'>" + moment.utc(1000 * queued_t).fromNow() + "</span>"
                  //"updated "+moment.utc(source.dateupdated).fromNow()
                ];

                if (source.datestarted) {
                  var started_t = moment.utc(source.datestarted).valueOf() / 1000;
                  display.push("started  <span title='" + moment.utc(1000 * started_t).fromNow() + dms.date(' Y-m-d H:i:s', started_t) + "'>" + moment.utc(1000 * started_t).fromNow() + "</span>");
                }
                if (source.totaltime) {
