			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
				{
					...
					var fnClickHandler = function ( e ) {
						e.data.actionCall && (e.data.action = e.data.actionCall());
					...
					$(nPaging).append(
						...
						'</ul>' + 
						'<span class="el-page-area"><input type="text" class="el-page-input"> <button class="el-page-go">GO</button> </span>' + 
						'<span class="el-page-num">0 页</span>'
					);
					var els = $('a', nPaging);
					var nFirst = els[0],
						nPrev = els[1],
						nNext = els[2],
						nLast = els[3],
						nGo = $('.el-page-go', nPaging);
					oSettings.oApi._fnBindAction( nFirst, {action: "first"},    fnClickHandler );
					oSettings.oApi._fnBindAction( nPrev,  {action: "previous"}, fnClickHandler );
					oSettings.oApi._fnBindAction( nNext,  {action: "next"},     fnClickHandler );
					oSettings.oApi._fnBindAction( nLast,  {action: "last"},     fnClickHandler );
					oSettings.oApi._fnBindAction( nGo,  {actionCall: function(){ return parseInt($('.el-page-input', nPaging).val()) - 1; }},     fnClickHandler );
					...
					for ( i=0, iLen=an.length ; i<iLen ; i++ )
					{
						...
						$(sList)
							...
							.bind('click', function (e) {
								...
								if ( oSettings.oApi._fnPageChange(oSettings, parseInt(page.replace(/,/g, ''))-1) ) {
									fnCallbackDraw( oSettings );
								}
							});
					}
