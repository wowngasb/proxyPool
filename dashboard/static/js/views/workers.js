      ...
      _.extend(datatableConfig, {
        "aoColumns": [
          ...
          },
          {
            ...
            "mData":function(source, type/*, val*/) {
              if (type == "display") {
                var datereported_t = moment.utc(source.datereported).valueOf() / 1000;
                var datereported_s = source.datereported ? moment.utc(source.datereported).fromNow() : "Never";
                var started_t = moment.utc(source.datestarted).valueOf() / 1000;
                var started_s = moment.utc(source.datestarted).fromNow();
                return "<small title='datereported " + datereported_s + dms.date(' Y-m-d H:i:s', datereported_t) + "'>" + datereported_s
                   + "</small><br/><small title='started " + started_s + dms.date(' Y-m-d H:i:s', started_t) + "'>started " + started_s + "</small>";
              } else {
